...
import { Belonging, BelongingType, SheetResource, SheetResourceType, SheetType } from '../../../models/sheet';
    ...
    <ModalForm nested={nested} type="alert" title={data.title || 'Are you sure?'} submitText={data.submitText || 'Yes, I want to delete this item'} submitHandler={submitHandler}>
      <div className="mt-2 space-y-4">
        ...
        {(data.resource as Belonging).equipped ? (
          <Notice noIcon status={NoticeStatus.Error} message={`Deleting this ${getBelongingType(data.resourceType as unknown as BelongingType)} will also unequip it.`} />
        ) : null}
      </div>
    </ModalForm>
  );
