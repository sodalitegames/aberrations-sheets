...
export const ToggleSwitch = ({ name, enabled, setEnabled }) => {
  return (
    <Switch
      checked={enabled}
      onChange={setEnabled}
    ...
    >
      ...
      >
        ...
        <span
          ...
          aria-hidden="true"
        >
          <svg className="h-3 w-3 text-gray-900" fill="currentColor" viewBox="0 0 12 12">
...
const Toggle = ({ label, enabled, setEnabled, name, slideOver }) => {
  return (
    <Row name={name} label={label} slideOver={slideOver}>
      <div className="pt-2">
        <ToggleSwitch name={name} enabled={enabled} setEnabled={setEnabled} />
      </div>
    </Row>
  );
};
